...
export class HandUI {
  ...
  private resizeHandContainer() { // Also reposition it
    let newWidth = 0
    let newHeight = 0
    let index = 0

    this.handContainer.iterate((child: Phaser.GameObjects.Container) => {
      child.setX(child.width * child.scaleX * index)
      newWidth += child.width * child.scaleX
      newHeight = child.height * child.scaleY + 35 // Adjusted because black border
      index++
    })
